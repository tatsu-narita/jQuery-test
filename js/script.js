...
フォームに入力または選択されている値(value属性)を取得・変更できる

[value属性の値を取得]
val()は括弧内に何も記述せずに使用せずに使用すると
フォームに入力されているテキストや選択されている値を取得します
値を取得する対象要素はセレクターで指定します
例えばセレクターでinput要素を指定するとval()はフォーム部品の種類を決める
type属性に関係なく全てのinput要素のvalue属性の値を取得します

val()はinput要素だけでなく
select/option要素やtextarea要素に対しても使用できます
例えばセレクターでselect要素を指定すると
選択されているoption要素のvalue属性の値を取得できる

$("select").val();

value属性を持たないtextarea要素の場合入力されているテキストを取得します
...
//value属性の値変更
/*
フォームの内容を変更するときもval()を使う
対象とする要素はセレクターで指定し変更したい値を括弧内に記述します
これはクリアボタン(button要素)がクリックされたらval("")を実行し
input要素の値を空に変更(削除)するスクリプトです
*/
/*
$(function () {
    $("button").click(function () { 
...
//フォームに関するイベントの処理
/*
jqueryにはフォーム部品の操作によって発生したイベントを
感知する命令が用意されている
2-3で紹介した他のイベントと組み合わせると
ユーザーの操作に反応するインタラクティブなフォームを実現する

[フォーカスを感知するfocus()]
これはinput要素などのフォーム部品がマウスやタブキーによって
選択された状態になった事を感知し設定された処理を実行する
他のイベントと同じように括弧内にfunction(){}を使いその中に実行したい処理を記述する

これはinput要素のvalue属性の値がからになりテキストは削除される
同時にinput要素のcssを操作し文字色を黒に変更します
*/
/*
$(function() {
...
/*
ただしこれはinput要素がフォーカスされるたびに
フォームを空にする処理が実行されるため
ユーザーが入力した内容が削除される場合がある
実用上はイベント発生時に一度だけ命令を実行するone()イベントを併用し
focus()イベントが発生した初回のみ処理を実行するようにすると良い
 */
/*
$(function() {
    $("input").val(" 入力してください").css("color", "#ccc")
...
/*
blur()はfocus()とは逆に
フォーム部品からフォーカスが外れた(非選択状態になった)事を
感知して命令を実行する

これは前のコードにメソッドチェーンでblur()を追加したものです
input要素のvalue属性に(入力してください)と書き込んでおき
input要素がフォーカスされると値を削除する
フォーカスが外れたときinput要素がフォームが入力入力されているか否か
(input要素のvalue属性の値の有無)によって異なる処理をします
value属性が空の場合は最初と同じイベントの処理を再設定し
value属性に値が入っていれば何もしません

フォームへの入力・未入力によって処理を変えるにはif文構文を使う
*/
/*
$(function() {
    $("input").val(" 入力してください").css("color", "#ccc")
...
//フォークの内容変更を感知する change()
/*
フォーム内容(フォーム部品の値)が変更された事を感知する命令
内容が変更されたかどうかは該当するフォーム部品から
フォーカスが外されたタイミングで判断されます

これはセレクトボックスの選択行が変更されると
option要素のvalue属性の値をval()で読み出し
読み出した値をspan要素に書き込みます
option要素のvalue属性にはお名前、会社名というラベル名を付けてあるので
セレクトボックスの選択行に合わせてフォームの左横にあるラベルが変更されます
*/
/*
$(function() {
...
//フォームの送信を感知する submit()
/*
フォームの送信ボタンがクリックされた時に発生するsubmitイベントの処理を設定します
webブラウザーはフォームの送信ボタン(type属性がsubmitもしくはimageのinput要素)
がクリックされると送信を包んでいるform要素に記述されているaction属性のurlに対して
form要素内のフォーム部品で入力・選択されている情報を送信します
例えば次のhtmlの場合、送信ボタンをクリックするとフォーム部品の値がcheck.htmlに送信される

送信ボタンがクリックされた後、データをaction属性のURLに送信する前に
処理を実行するのがsubmit()です
設定した処理を実行し終えるとaction属性のURLにデータが送信されますがsubmitイベントの
処理の中でreturn false;を追加すると送信をキャンセルできます

以下はHTMLの問合せフォームにチェック機能を追加するスクリプトです
送信ボタンをクリックされると入力必須項目である(お名前[name属性の値がnameのinput要素])
の値をval()で取得し、入力・未入力をif文で調べます
未入力の場合は、input要素のborderプロパティを垢に変更し
input要素の後ろに「名前を入力してください」と言うspan要素を挿入、
span要素のcolorプロパティを赤色にfont-weightプロパティをboldにします
*/
...
/*
ただこのままだと送信ボタンをクリックするたびに「お名前を入力してください」
と言うテキストが何度も表示されてしまいます
...
一つは二つ上で使ったinput要素の値を調べて処理を分岐するif文。
もう一つはspan要素のfont-weightプロパティの値を調べて処理を分岐するif文。
そこではinput要素が未入力(空)の時にspan要素を挿入して
